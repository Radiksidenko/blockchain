...
class Blockchain(object):
    ...
    def __init__(self, _genesisBlock, account):
        """
            Если блок Genesis действителен, создайте цепочку
        """
        self.blockChain = []
        self.tempBlocks = []
    ...
    def generate_new_block(self, bpm=randint(0, 30688), oldBlock='', address=''):
        if self.myCurrBlock:
    ...
    def pos(self):
        ...
        print(str(self.myAccount) + ' =======================> Получение действительной цепочки\n')
        self.resolve_conflict()
        time.sleep(1)
        self._pos()
        print('***Вызов других узлов для объявления их***' + "\n")
        time.sleep(1)
        for node in self.nodes:
            node._pos()
        time.sleep(1)
    ...
    def _pos(self):
        print("Account ==========================> " + str(self.myAccount) + "\n")
        time.sleep(1)
        print('***Генерация нового блока PoS***' + "\n")
        time.sleep(1)
        self.generate_new_block()
        print('***Обмен временными блоками с другими узлами***' + "\n")
        time.sleep(1)
        self.get_blocks_from_nodes()
        print('***Выбор победителя***' + "\n")
        time.sleep(1)
        print("победитель =======================> " + str(self.pick_winner()) + "\n")
...
def main():
    ...
    blockchain5 = Blockchain(GENESIS_BLOCK4, account5)
